...
#ifndef DEBUG
	UNREFERENCED_PARAMETER(text); // Avoid unused parameter warning
#endif // DEBUG

	KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, text)); // Fixed typo in DPFLTR_IHVDRIVER_ID
}
...
NTSTATUS real_main(PDRIVER_OBJECT driver_obj, PUNICODE_STRING registery_path) {
	...
	RtlInitUnicodeString(&dev_name, L"\\Device\\baredriver"); //die lit
	...
	RtlInitUnicodeString(&sym_link, L"\\DosDevices\\baredriver");
...
extern "C" NTSTATUS DriverEntry(PDRIVER_OBJECT driver_obj, PUNICODE_STRING registery_path) {
	...
	debug_print("[+] Bare Driver Loaded!\n");

	UNICODE_STRING  drv_name;
	RtlInitUnicodeString(&drv_name, L"\\Driver\\baredriver");
